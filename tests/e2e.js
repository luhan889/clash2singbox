...
var PROBE = [
	...
	'hasIn:(st.json||"").indexOf("\\"inbounds\\"")>=0,stat:q("#statNodes").textContent,clientHidden:(function(){var f=q("#opt-preset"),l=f&&f.closest?f.closest("label"):null;return !f||!l||!!l.hidden})(),chip:(function(){var p=q("#protos .proto");return p?p.textContent:""})(),chipDot:!!q("#protos .proto i"),groupsOpen:(function(){var d=document.querySelectorAll("#rail details.group"),n=0;for(var i=0;i<d.length;i++)if(d[i].open)n++;return n})(),toastTop:(function(){var t=q("#toasts"),h=q(".bar");if(!t||!h)return -1;return Math.round(t.getBoundingClientRect().top-h.getBoundingClientRect().bottom)})(),toastRight:(function(){var t=q("#toasts");return t?Math.round(window.innerWidth-t.getBoundingClientRect().right):-1})(),headHit:(function(){var h=q(".rail-head");if(!h)return "none";var r=h.getBoundingClientRect();var e=document.elementFromPoint(Math.round(r.left+r.width/2),Math.round(r.top+r.height/2));return e?((e===h||h.contains(e))?"head":((e.className||e.tagName)+"")):"none"})(),bodyScroll:(function(){var b=q("#railBody");return b?b.scrollTop:-1})(),bodyCanScroll:(function(){var b=q("#railBody");return !!(b&&b.scrollHeight>b.clientHeight)})(),railScroll:(function(){var b=q("#rail");return b?b.scrollTop:-1})(),vw:window.innerWidth,vh:window.innerHeight,railCls:(function(){var r=q("#rail");return r?r.className:""})(),railTop:(function(){var r=q("#rail");return r?Math.round(r.getBoundingClientRect().top):-1})(),noneBlock:!!q("#issues .none"),drawerHidden:(function(){var d=q("#issues");return !d||!!d.hidden})(),issueRows:document.querySelectorAll("#issues .issue").length,sam:!!q("#issues details.sam"),samOpen:(function(){var d=q("#issues details.sam");return !!(d&&d.open)})(),fix:!!q("#issues [data-fix=target]"),toasts:document.querySelectorAll("#toasts .toast").length,act:!!q("#toasts .toast .act"),tgt:(function(){var f=q("#opt-target");return f?f.value:""})(),inW:(function(){var p=q("#paneIn");return p?Math.round(p.getBoundingClientRect().width):-1})(),inH:(function(){var p=q("#paneIn");return p?Math.round(p.getBoundingClientRect().height):-1})(),shellCls:(function(){var s=q(".shell");return s?s.className:""})(),ovf:document.documentElement.scrollWidth-window.innerWidth,splitStore:localStorage.getItem("c2s.split")||"",splitVis:(function(){var h=q("#split2");return h?getComputedStyle(h).display:"none"})()}}',
	...
	'step("resetOptions",function(){q("#reset").click()});',
	'step("afterOptionsReset",function(){R.steps.afterOptionsReset=snap()});',
...
for (var t = 0; t < targets.length; t++) {
	...
	ok(label + '：仅基础选项默认展开', !!s.boot && s.boot.groupsOpen === 1, s.boot && s.boot.groupsOpen)
	...
	ok(label + '：侧栏溢出时仅内容区滚动', !!s.afterScroll && (!s.afterScroll.bodyCanScroll || s.afterScroll.bodyScroll > 0) && s.afterScroll.railScroll === 0, s.afterScroll && [s.afterScroll.bodyCanScroll, s.afterScroll.bodyScroll, s.afterScroll.railScroll])
	...
	ok(label + '：通知位于右上且不遮挡顶栏', !!s.afterPaste && s.afterPaste.toastTop >= 10 && s.afterPaste.toastRight >= 10 && s.afterPaste.ovf <= 1, s.afterPaste && [s.afterPaste.toastTop, s.afterPaste.toastRight, s.afterPaste.ovf])
	...
	ok(label + '：恢复默认后仍仅展开基础选项', !!s.afterOptionsReset && s.afterOptionsReset.groupsOpen === 1, s.afterOptionsReset && s.afterOptionsReset.groupsOpen)
}
