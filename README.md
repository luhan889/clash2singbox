...
### 直接使用
​
下载或克隆源码后，直接双击 `index.html` 即可。**无需安装依赖，也无需构建。**
​
> 在 `file://` 环境下，浏览器不允许创建 Web Worker，应用会自动回退到主线程转换；核心转换功能不受影响。
​
### 启动本地服务器（推荐）
​
如需启用 Web Worker、PWA 等完整浏览器能力，可任选一种方式启动静态服务器：
​
```bash
# Node.js；首次运行时 npx 可能会临时下载 serve
npx serve .
​
# 或使用 Python 3
